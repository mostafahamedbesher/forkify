...
class RecipeView extends View {
  ...
  addHandlerAddBookmark(handler) {
    this._parentElement.addEventListener('click', e => {
      const btnBookmark = this._parentElement.querySelector('.btn--bookmark');

      if (e.target.closest('.btn--bookmark') === btnBookmark) {
        handler();
      }
    });
  }
  ...
  addHandlerServings(handler) {
    this._parentElement.addEventListener('click', e => {
      ...
      const updateTo = Number(btnServing.dataset.updateTo);
